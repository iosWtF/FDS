...
@interface BMEditDeviceViewController ()<UITableViewDelegate,UITableViewDataSource,TZImagePickerControllerDelegate,UIImagePickerControllerDelegate,UIAlertViewDelegate,UINavigationControllerDelegate,STPickerSingleDelegate,STPickerDateDelegate>
...
@implementation BMEditDeviceViewController
#pragma mark ======  lazy  ======
...
- (void)viewDidLoad {
    ...
    [self.tableView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(- SafeAreaBottomHeight - 85 * AutoSizeScaleY);
    }];
...
- (void)pickerSingle:(STPickerSingle *)pickerSingle selectedTitle:(NSString *)selectedTitle
{
    ...
    NSArray * arrayData = @[@"学前班",@"一年级",@"二年级",@"三年级",@"四年级",@"五年级",@"六年级",@"初中",@"高中",@"其他"];
    self.device.childrenType = [NSString stringWithFormat:@"%lu",(unsigned long)[arrayData indexOfObject:selectedTitle]];
    BMEditDeviceTableViewCell * cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:3 inSection:0]];
    cell.content = selectedTitle;
}

- (void)pickerDate:(STPickerDate *)pickerDate year:(NSInteger)year month:(NSInteger)month day:(NSInteger)day
{
    NSString *text = [NSString stringWithFormat:@"%zd-%zd-%zd 00:00:00", year, month, day];
    NSString *text1 = [NSString stringWithFormat:@"%zd-%zd-%zd", year, month, day];
    self.device.birthday = text1;
    BMEditDeviceTableViewCell * cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:2 inSection:0]];
    cell.content = text1;
}
...
- (void)upLoadImage{
    ...
    [[BMHttpsMethod httpMethodManager] fileUploadFileWithImgArr:@[self.iconImg] ToGetResult:^(id  _Nonnull data) {
        ...
        if ([data[@"errorCode"] isEqualToString:@"0000"]) {
            ...
            self.device.picUrl = data[@"data"][@"path"];
            [self keepInfo];
        }else{
...
- (void)keepInfo{
    ...
    if (!self.iconImg || self.device.picUrl.length == 0) {
        
        [self showHint:@"请上传设备图片"];
        return;
    }
    ...
    if (self.device.birthday.length == 0) {
        
        [self showHint:@"请选择儿童生日"];
        return;
    }
    if (self.device.childrenType.length == 0) {
        
        [self showHint:@"请选择儿童学习阶段"];
        return;
    }
    [[BMHttpsMethod httpMethodManager] deviceUpdateWithId:self.device.Id Name:self.device.name Birthday:self.device.birthday ChildrenType:self.device.childrenType PicUrl:self.device.picUrl ToGetResult:^(id  _Nonnull data) {
        ...
        if ([data[@"errorCode"] isEqualToString:@"0000"]) {
            ...
            for (UIViewController * vc in self.navigationController.viewControllers) {
                ...
                if ([vc isKindOfClass:[BMMapViewController class]]) {
                    
                    BMMapViewController * map = (BMMapViewController *)vc;
                    [self.navigationController popToViewController:map animated:YES];
                   
                }else if ([vc isKindOfClass:[BMMineGroupInfoViewController class]]) {
                    
                    BMMineGroupInfoViewController * group = (BMMineGroupInfoViewController *)vc;
                    [self.navigationController popToViewController:group animated:YES];
                    
                }else if ([vc isKindOfClass:[BMGroupInfoViewController class]]) {
                    
                    BMGroupInfoViewController * group = (BMGroupInfoViewController *)vc;
                    [self.navigationController popToViewController:group animated:YES];
                    
                }else if ([vc isKindOfClass:[BMMineViewController class]]) {
                    
                    BMMineViewController * mine = (BMMineViewController *)vc;
