...
@interface BMChangePhoneViewController ()<UITextFieldDelegate>
...
@implementation BMChangePhoneViewController
...
- (void)confirm{
    ...
    [[BMHttpsMethod httpMethodManager] userUpdatePhoneNoWithUserId:UserID PhoneNo:self.phoneNumTf.text CheckCode:self.codeTf.text ToGetResult:^(id  _Nonnull data) {
       
        if ([data[@"errorCode"] isEqualToString:@"0000"]){
            
            [self showHint:@"更换手机号成功"];
            for (UIViewController * vc in self.navigationController.viewControllers) {
                
                if ([vc isKindOfClass:[BMMineSetViewController class]]) {
                    
                    BMMineSetViewController *  set = (BMMineSetViewController *)vc;
                    [self.navigationController popToViewController:set animated:YES];
                }
            }
...
- (void)setupUI{
    ...
    phoneNumTf.font = [UIFont systemFontOfSize:16.f];
    ...
    ZViewBorderRadius(confirmBtn, 5, 0, [UIColor getUsualColorWithString:@"#FF5445"]);
    [confirmBtn addTarget:self action:@selector(confirm) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:confirmBtn];
    [confirmBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        
        make.centerX.equalTo(self.view);
        make.bottom.equalTo(- 20 * AutoSizeScaleY - SafeAreaBottomHeight);
        make.size.equalTo(CGSizeMake(320 * AutoSizeScaleX, 45 * AutoSizeScaleY));
    }];
