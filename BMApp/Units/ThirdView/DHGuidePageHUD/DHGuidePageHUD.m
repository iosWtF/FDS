...
//        [self addSubview:skipButton];
        ...
        for (int i=0; i<imageNameArray.count; i++) {
            ...
            [imageView sd_setImageWithURL:[NSURL URLWithString:imageNameArray[i]] placeholderImage:nil];
...
//            }
            ...
            if (i == imageNameArray.count-1 && isHidden == NO) {
                ...
                UIButton *startButton = [[UIButton alloc]initWithFrame:CGRectMake(Z_SCREEN_WIDTH / 2 - 59, DDScreenH*0.8 + 34, 118, 34)];
...
//                ZViewBorderRadius(startButton, 5.0, 0, [UIColor whiteColor]);
//                [startButton setBackgroundColor:[UIColor blackColor]];
                [startButton setBackgroundImage:[UIImage imageNamed:@"yindao_btn_kaiqi"] forState:UIControlStateNormal];
        ...
        self.imagePageControl.pageIndicatorTintColor = [UIColor getUsualColorWithString:@"#ffcac3"];
        self.imagePageControl.currentPageIndicatorTintColor = [UIColor getUsualColorWithString:@"#ffffff"];
...
- (void)buttonClick:(UIButton *)button {
    [UIView animateWithDuration:DDHidden_TIME animations:^{
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(DDHidden_TIME * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self performSelector:@selector(removeGuidePageHUD) withObject:nil afterDelay:0.5];
        });
    }];
}
...
- (instancetype)dh_initWithFrame:(CGRect)frame videoURL:(NSURL *)videoURL {
    if ([super initWithFrame:frame]) {
        ...
        // 视频引导页进入按钮
        UIButton *movieStartButton = [[UIButton alloc] initWithFrame:CGRectMake(20, DDScreenH-30-60, DDScreenW-60, 40)];
