...
- (void)setupUI {
    ...
    self.cq_titleLabel.textColor = [UIColor getUsualColorWithString:@"#666666"];
    [contentView addSubview:self.cq_titleLabel];
    self.cq_titleLabel.font = [UIFont systemFontOfSize:13];
    ...
    self.cq_arrowImageView.image = [UIImage imageNamed:@"icon_paixun"];
...
- (void)setSelected:(BOOL)selected {
    ...
    if (selected) { // 即将设置成选中状态
        self.cq_titleLabel.textColor = [UIColor getUsualColorWithString:@"#FF564C"];
        if (super.selected) { // 如果原本就处于选中状态
            // 那么就切换筛选状态
            _ascending = !_ascending;
            if (_ascending) {
                // 升序
                self.cq_arrowImageView.image = [UIImage imageNamed:@"icon_paixun_up"];
            } else {
                // 降序
                self.cq_arrowImageView.image = [UIImage imageNamed:@"icon_paixun_down"];
            }
        } else { // 如果之前不是选中状态
            // 那么设置成选中的默认排序状态：升序
            _ascending = YES;
