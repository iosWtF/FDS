...
- (void)setLayerFrame:(CGRect)frame
{
    ...
    self.popLayer.fillColor = _layerColor? _layerColor.CGColor : [UIColor getUsualColorWithString:@"#282828"].CGColor;
}
...
@property (nonatomic, strong) NSArray *groupMenus;
...
- (instancetype)initWithBounds:(CGRect)bounds groupMenus:(NSArray *)groups config:(PopOverVieConfiguration *)config
{
    self = [super initWithFrame:bounds];
    if (self) {
        if (!config) {
...
- (void)initDefaultConfig
{
    ...
    _config.triAngelHeight = 5.0;
    _config.triAngelWidth = 10.0;
    _config.containerViewCornerRadius = 0;
    _config.roundMargin = 0;
    _config.showSpace = 2.0;
    
    // 普通用法
    _config.defaultRowHeight = 40.f;
    _config.tableBackgroundColor = [UIColor getUsualColorWithString:@"#282828"];
    _config.separatorColor = [UIColor getUsualColorWithString:@"#FFFFFF"];
    _config.separatorStyle = UITableViewCellSeparatorStyleSingleLine;
    _config.textColor = [UIColor getUsualColorWithString:@"#FFFFFF"];
    _config.font = [UIFont systemFontOfSize:12.f];
    _config.textAlignment = NSTextAlignmentCenter;
}

- (void)setHomeModel:(BMHomeModel *)homeModel{
    
    _homeModel = homeModel;
    self.groupMenus = homeModel.clusterList;
    self.isGroup = YES;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.isGroup) {
        ...
        BMClusterModel * model = self.groupMenus[indexPath.row];
        BMGroupListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
        if (nil == cell) {
            cell = [[BMGroupListTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            cell.contentView.backgroundColor = [UIColor clearColor];
        }
        cell.model = model;
        if ([model.Id isEqualToString:self.homeModel.clusterId]) {
