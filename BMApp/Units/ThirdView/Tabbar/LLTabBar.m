...
- (void)config {
	...
	UIImageView *topLine = [[UIImageView alloc] initWithFrame:CGRectMake(0, -5, Z_SCREEN_WIDTH, 5)];
...
- (void)setTabBarItemAttributes:(NSArray<NSDictionary *> *)tabBarItemAttributes {
    ...
    CGFloat itemWidth = Z_SCREEN_WIDTH / _tabBarItemAttributes.count;
...
- (LLTabBarItem *)tabBarItemWithFrame:(CGRect)frame title:(NSString *)title normalImageName:(NSString *)normalImageName selectedImageName:(NSString *)selectedImageName tabBarItemType:(LLTabBarItemType)tabBarItemType {
    ...
    item.titleLabel.font = [UIFont systemFontOfSize:12.f weight:0];
    UIImage *normalImage = [UIImage imageNamed:normalImageName];
    UIImage *selectedImage = [UIImage imageNamed:selectedImageName];
    [item setImage:normalImage forState:UIControlStateNormal];
    [item setImage:selectedImage forState:UIControlStateSelected];
    [item setTitleColor:[UIColor getUsualColorWithString:@"#666666"] forState:UIControlStateNormal];
    [item setTitleColor:[UIColor getUsualColorWithString:@"#4285F4"] forState:UIControlStateSelected];
