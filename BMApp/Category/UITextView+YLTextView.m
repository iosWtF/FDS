...
- (UIFont *)limitPlaceFont {
    return objc_getAssociatedObject(self, &LIMITFONT) == nil ? [UIFont systemFontOfSize:16.] : objc_getAssociatedObject(self, &LIMITFONT);
}
...
- (UIFont *)placeholdFont {
    return objc_getAssociatedObject(self, &PLACEHOLDFONT) == nil ? [UIFont systemFontOfSize:16.] : objc_getAssociatedObject(self, &PLACEHOLDFONT);
}
...
- (UIColor *)placeholdColor {
    return objc_getAssociatedObject(self, &PLACEHOLDCOLOR) == nil ? [UIColor getUsualColorWithString:@"#9A9A9A"] : objc_getAssociatedObject(self, &PLACEHOLDCOLOR);
}
...
- (UIColor *)limitPlaceColor {
    return objc_getAssociatedObject(self, &LIMITCOLOR) == nil ? [UIColor getUsualColorWithString:@"#9A9A9A"] : objc_getAssociatedObject(self, &LIMITCOLOR);
}
...
- (void)setPlaceHolderLabel:(NSString *)placeholder {
    ...
    CGRect rect = [placeholder boundingRectWithSize:CGSizeMake(CGRectGetWidth(self.frame), CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: self.font} context:nil];
    self.placeholderLabel.frame = CGRectMake(12 * AutoSizeScaleX, 8 * AutoSizeScaleY, rect.size.width - 24 * AutoSizeScaleX, rect.size.height);
...
- (void)setWordcountLable:(NSNumber *)limitLength {
     ...
     *  字数限制
     */
    self.wordCountLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.frame) - 86 * AutoSizeScaleX,CGRectGetHeight(self.frame) - 28 * AutoSizeScaleY , 80 * AutoSizeScaleX, 20 * AutoSizeScaleY)];
...
- (void)textViewChanged:(NSNotification *)notification {
    ...
    if ([[[self textInputMode] primaryLanguage] isEqualToString:@"emoji"] || ![[self textInputMode] primaryLanguage]) {
        ...
        self.text = [self filterEmoji:self.text];
    }
