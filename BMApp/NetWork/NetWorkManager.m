...
+(void)requestWithType:(HttpRequestType)type withUrlString:(NSString *)urlString withParaments:(id)paraments withShowIndicator:(BOOL)show withSuccessBlock:( requestSuccess)successBlock withFailureBlock:( requestFailure)failureBlock progress:(downloadProgress)progress
{
...
//            UIImage *  resizedImage =  [UIImage IMGCompressed:image targetWidth:width];
            ...
            //拼接data
            [formData appendPartWithFileData:imgData name:[NSString stringWithFormat:@"file"] fileName:@"image.png" mimeType:@"image/jpeg"];
 ...
 *
 */
+(void)uploadImageWithOperations:(NSDictionary *)operations withImage:(UIImage *)image withUrlString:(NSString *)urlString withSuccessBlock:(requestSuccess)successBlock withFailurBlock:(requestFailure)failureBlock withUpLoadProgress:(uploadProgress)progress
{
